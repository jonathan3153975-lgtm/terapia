          ...
          <tbody id="myContentsTableBody">
            ...
            <?php else: ?>
              <?php foreach ($favoriteBooks as $book): ?>
                <tr class="my-contents-data-row" data-search="<?php echo htmlspecialchars(strtolower((string) ($book['title'] ?? ''))); ?>">
...
window.addEventListener('load', function () {
  ...
  var dataRows = Array.prototype.slice.call(document.querySelectorAll('.my-contents-data-row'));
  var tableBody = document.getElementById('myContentsTableBody');
  var emptyRow = document.getElementById('myContentsEmptyRow');
  var paginationInfo = document.getElementById('myContentsPaginationInfo');
  var paginationControls = document.getElementById('myContentsPaginationControls');
  var paginationWrapper = document.getElementById('myContentsPaginationWrapper');
  var rowsPerPage = 10;
  var currentPage = 1;

  var clearDynamicEmptyRow = function () {
    var existingDynamicEmpty = document.getElementById('myContentsNoSearchMatchRow');
  ...
  var applyFiltersAndPagination = function () {
    ...
    if (totalItems === 0 && tableBody) {
      ...
      tr.id = 'myContentsNoSearchMatchRow';
      tr.innerHTML = '<td colspan="3" class="text-center text-muted py-4">Nenhum conteúdo encontrado para a busca.</td>';
